...
def summer(t,r,a):
    ...
    n = len(a)
    for i in range(0,n):
        temp+= a[i]*cmath.exp(1j*t)*((r*cmath.exp(1j*t))**(a[i]-1))/(1-(r*cmath.exp(1j*t))**a[i])
    return temp
...
def H(a,t,R,gamma):
    ...
    on the parametrized variable t and the gamma function.'''
...
def H_ellipse(a,t,r1,r2,gamma):
    '''The ellipse method requires two radii, also takes the gamma function and the a vector'''
    temp=1
    n=len(a)
    for k in range(0,n):
...
def H_radial(a,R,x0,x1,y0,y1,gamma):
    '''Used for integrals along a ray from the inside to outside of the circle, perpendicular to the
...
### Function to retrieve Pisinger Instances ###
...
def read_pisinger_file(f):
    ...
    for line in f:
        ...
        if l[0] == '-----':
            ...
            suba=[]
            start = False
        elif l[0].startswith('time'):
            start = True
        elif start:
            suba.append(int(l[0].split(',')[2]))
