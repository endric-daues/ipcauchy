...
def evaluate_integral_ellipse(a,b,alpha,beta,R1,R2,gamma,gamma_der,error):
    ...
    with an adaptive quadrature approach.'''
    
    val = quadpy.line_segment.integrate_adaptive(lambda t: integrand_ellipse_adaptive(t,a,b,R1,R2,gamma,gamma_der),[alpha,beta],error)
    return ((1/(2*math.pi*1j)) * val[0],val[1])


def shortest_path_arc_integral(start_radius,radius_delta,N,a,b,error):
    '''Solves the shortest path given a,b and outputs the integral, moves along arcs not straight lines
    The max radius should be larger than the min radius such that there are not
    zero paths. THe precision of the max_radius should be larger than the delta
    since this sometimes throws an error.'''
