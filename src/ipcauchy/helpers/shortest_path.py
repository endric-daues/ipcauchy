...
def H_mpc(a, z):
    ...
    the current position on the parametrized variable t and
    the gamma function. Uses MPC and is not vectorized.
    '''
    temp = 1
    n = len(a)
    for k in range(0, n):
...
def dist(p1, p2):
    ...
    imaginary numbers in the plane
    '''
    return math.sqrt(
        (p1.real-p2.real) ** 2 + (p1.imag - p2.imag) ** 2
    )


def dist2(p1, p2):
    '''
    Returns the arc distance between the two imaginary
...
def edge_value(p1, p2, a, b):
    '''
    Determines the cost of the edge between
    points p1 and p2 given a,b
    '''
...
def edge_value2(p1, p3, p2, a, b):
    '''
    Determines the cost of the edge between points
    p1 and p2 given a,b using the three point rule
    '''
    return (
        abs(
            0.5
...
def generate_graph(radii, angles):
    '''
    Generates the graph on which the shortest path
...
def fill_edges(graph, radii, angles, nodes, a, b):
    '''
    Fills the edges of the given graph with weights using the
    three point rule and adds them to the Dijkstar library
    '''
